...
function bursts_list = identifybursts(trace_name, threshold_slope)
    ...
    data=dataallsweeps(:,1,1); %data to analyze
    ...
    restingpotential = data(1); %resting potential. used for calculating AHP troughs
     ...
     if (numel(all_dV) ~= 0) 
        ...
        if max(diff(all_dV)) > w %look for spikes that are at least [avg. width of an AP] apart
            for i = 2:numel(all_dV) %
                if all_dV(i) - all_dV(i-1) < w
                    continue
        ...
        display(spiketimes_t); %these are the spike times
