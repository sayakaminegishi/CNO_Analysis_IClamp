function [P, TBL, STATS] = tableANOVA_WKY_NeuronsOnly_vs_NeuronsPlusGlia_AllFitless_Ranked(biggestTable)
% TABLEANOVA_WKY_NEURONSONLY_VS_NEURONSPLUSGLIA_ALLFITLESS_RANKED
% ANOVA on RANKS comparing (within WKY only):
%   Group A: WKY Neurons ONLY
%   Group B: WKY Neurons + Glia
%
...
% -------------------------------------------------------------------------
% 3) Apply Global Filters (WKY, No Virus, No CNO)
% -------------------------------------------------------------------------
...
group_base(has_neuronF & ~has_gliaF) = "WKY Neurons only";
group_base(has_neuronF &  has_gliaF) = "WKY Neurons + Glia";
...
% -------------------------------------------------------------------------
figure('Name', 'WKY: Neurons only vs Neurons+Glia (Ranked)', ...
       'Units', 'normalized', 'Position', [0.05 0.05 0.9 0.85]);
t = tiledlayout(2, 3, 'TileSpacing', 'compact', 'Padding', 'compact');
title(t, 'WKY: Neurons only vs Neurons + Glia (No Virus/DREADDs; No CNO; Rank-transformed ANOVA)', ...
...
for i = 1:numel(fitless_vars)
    ...
    n_neur = sum(g_clean == "WKY Neurons only");
    n_ng   = sum(g_clean == "WKY Neurons + Glia");
    if n_neur == 0 || n_ng == 0
        continue;
    end
    ...
    % --- Labeling with n ---
    l_neur = sprintf('WKY Neurons only (n=%d)', n_neur);
    l_ng   = sprintf('WKY Neurons+Glia (n=%d)', n_ng);

    g_final = strings(size(g_clean));
    g_final(g_clean == "WKY Neurons only") = l_neur;
    g_final(g_clean == "WKY Neurons + Glia") = l_ng;
...
text(0.05, 0.66, 'Strain = WKY', 'Interpreter', 'none');
