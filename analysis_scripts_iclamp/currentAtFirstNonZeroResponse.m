function [p, tbl, stats, c, m, gnames] = currentAtFirstNonZeroResponse(biggestTable)
% TABLEANOVA2 - ANOVA on FI peak response: Strain (Filtered by Co-Culture)
%
% [P, TBL, STATS, C, M, GNAMES] = currentAtFirstNonZeroResponse(BIGGESTTABLE)
%
% Filters rows (virus/drug empty; celltype matches), runs one-way ANOVA
% on FIcalc.fitless.currentAtFirstNonZeroResponse grouped by StrainName, displays:
...
col_data     = 'FIcalc.fitless.currentAtFirstNonZeroResponse';
