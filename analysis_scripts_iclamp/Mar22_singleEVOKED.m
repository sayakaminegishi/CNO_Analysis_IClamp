...
filename = "2016_09_02_01_0008.abf"; %specify file to examine
...
current_injected = [-50:15:310]; %stimulus current
            ...
            threshold_value = data(all_dV_filtered(1)); %threshold for AP detection
                ...
                if(numel(pks_in_trace)>=1)
                    ...
                    [test_spike,starttime,endtime] = extract_waveform3(risingDuration,fallingDuration, mainpeakloc, data); %in sample units
                     ...
                     figure(8)
                plot(dur,test_spike)
                xlabel("time (sample units)")
                ylabel("membrane potential (mV)")

                 figure(10)
                plot(sampleunits_to_ms(si,dur),test_spike)
                xlabel("time (ms)")
                ylabel("membrane potential (mV)")
