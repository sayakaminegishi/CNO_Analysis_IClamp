function [P, TBL, STATS] = tableANOVA_WKY_CNO_AllFitless(biggestTable)
% TABLEANOVA_WKY_CNO_ALLFITLESS - Consolidated ANOVA for all fitless metrics
% 
% Features:
...
% -------------------------------------------------------------------------
% 3) Apply Global Filters (WKY, Virus-Present, Specific Cell Types)
% -------------------------------------------------------------------------
...
fprintf('\n--- Filtering Diagnostics (WKY + Virus + Co-Culture) ---\n');
...
if n_cno_total == 0 || n_ctrl_total == 0
    warning('blt:tableANOVA:InsufficientGroups', ...
        'Analysis aborted: One or both groups have 0 samples after filtering.');
    return; % Returns initialized empty/NaN values safely
end
...
% -------------------------------------------------------------------------
figure('Name', 'WKY Fitless Metrics: CNO vs Control (Virus Present)', ...
       'Units', 'normalized', 'Position', [0.05 0.05 0.9 0.85]);
t = tiledlayout(3, 3, 'TileSpacing', 'compact', 'Padding', 'compact');
title(t, 'ANOVA Comparison: WKY Strain (Virus + Co-Culture Present)', ...
...
for i = 1:numel(fitless_vars)
    ...
    % Run ANOVA (Display OFF to prevent new window spam)
    ...
    % Robust Labeling (Categorical prevents alphabetical reordering)
...
% ======================================================================
% Helper: whitespace/empty detector (Ported from tableANOVA2.m)
% ======================================================================
