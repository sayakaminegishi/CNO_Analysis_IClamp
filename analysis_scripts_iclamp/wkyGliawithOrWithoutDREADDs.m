function [stats, g, subTable] = wkyGliawithOrWithoutDREADDs(biggestTable)
% WKYGLIAVSDREADDS - Compare WKY neurons+glia without vs with DREADDs.
%
% Groups compared:
%   1) WKY + Neurons-Glia + no virus + no drug
%   2) WKY + Neurons-Glia + virus present + no drug
%
% Measurement:
%   FIcalc.fitless.peakResponse
    ...
    if isempty(subTable)
        error("wkyGliaVsDREADDs:EmptySubset", ...
            "No WKY + no-drug rows found.");
    end
    ...
    if isempty(subTable)
        error("wkyGliaVsDREADDs:NoGliaGroup", ...
            "No WKY neurons+glia rows found.");
    end
    ...
    if isempty(subTable)
        error("wkyGliaVsDREADDs:NoComparisonGroups", ...
    ...
    [stats, g, subTable] = blt.stats.anovaTool(subTable, ...
        ...
        'FigureTitle', "Peak Response in WKY Neurons+Glia: No DREADDs vs DREADDs");
