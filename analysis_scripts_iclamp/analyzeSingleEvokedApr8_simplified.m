function M1 = analyzeSingleEvokedApr8_simplified(filename1, current_injected1)
...
for a= 1: totalsweeps %analyze the sweep that invokes the first two APs in cell. change this to totalsweeps:totalsweeps to analyze 1st ap of last sweep
    if(apFoundInCell == 0)
        ...
        if(~isempty(all_dV_filtered))
            ...
            apFoundInCell = 1;
            threshold_voltage = data(all_dV_filtered(1)); %threshold for AP detection
            threshold_pt = all_dV_filtered(1); %point whre threshold first hits, in samples
            
            restingpotential = data(1); %resting potential.
            pks_in_trace = get_spikelocations(data,threshold_voltage); %get AP spike times in trace
            ...
            %TODO: include line of code to store the first sweep that
            %detected an AP
