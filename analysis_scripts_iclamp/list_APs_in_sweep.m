...
function [singleAPTimes, first_peak_burst, burst_times_matrix] = list_APs_in_sweep(sweep_data,dV_thresh)
...
for k = 2:numberOfSpikes
    ...
    if(spikeTimes(k) - spikeTimes(k-1) < w) %if the two spikes are a part of a burst
        ...
        AP_list(idx_AP) = spikeTimes(k); %count it as an AP if it's not part of a burst -> if AHP not present, then discard it later in analyze_singleAPs.m.
        idx_AP = idx_AP +1;
    end
end
