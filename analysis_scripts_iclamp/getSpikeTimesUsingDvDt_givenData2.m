function [apCounts, spikeTimes] = getSpikeTimesUsingDvDt_givenData2(dataallsweeps, timepoints)
% GETSPIKETIMESUSINGDVDT_GIVENDATA2 Detects and counts action potentials in each sweep
% using phase-plane analysis. Modified version that accepts data and timepoints
...
%   spikeTimes   : cell array of spike times (ms) for each sweep
...
for sweep = 1:numSweeps
    ...
    % Store spike times
    spikeTimes{sweep} = timepoints(peak_indices);
