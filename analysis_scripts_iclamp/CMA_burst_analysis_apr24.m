function [singletAnalysisRow_final, T] = CMA_burst_analysis_apr24(filename1)
...
%run burst detection for each sweep separately, then concatenate results at
%the end.
totalsweeps=size(dataallsweeps,3);


%parameters for each sweep
threshold_value = zeros(1,totalsweeps);
avg_isi = zeros(1,totalsweeps);
freq_in_hz = zeros(1,totalsweeps);
total_count_Aps = zeros(1,totalsweeps);
numbursts_list = zeros(1,totalsweeps);
count_singletAPs = zeros(1, totalsweeps);
avg_burst_duration = zeros(1, totalsweeps);
freq_burst_in_Hz = zeros(1, totalsweeps);
singletAnalysisRow_list = zeros(1, totalsweeps);
...
for sweep = 1:totalsweeps
    ...
    total_count_Aps(sweep) = 0; %total count of APs in this file
    thresh_list = []; %list of threshold from each sweep


    data=dataallsweeps(:,1,sweep); %data from sweep
    ...
    if(isempty(all_dV_filtered))
        ...
        threshold_value(sweep) = data(all_dV_filtered(1)); %threshold for AP detection
        ...
        total_count_Aps(sweep) = total_count_Aps(sweep) + numel(allSpikeSamples); %update total count of APs detected from this cell
        ...
        freq_in_hz(sweep) = total_count_Aps(sweep)/totalduration_sec;
        ...
        avg_burst_duration(sweep) = mean(burstdurationlist); %mean burst duration in ms (DOUBLE CHECK)

        count_singletAPs(sweep) = size(singletTimesMatrix, 1);
        freq_burst_in_Hz(sweep) = numbursts(1)/totalduration_sec;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        numbursts_list(sweep)= numbursts(1);

        avg_isi(sweep) = sum(isi_list)/numel(isi_list); %avg ISI across all sweeps, in ms
    ...
    singletAnalysisRow_list(sweep) = singletAnalysisRow;
end




%bursts - create table row for the cell


multipleVariablesRow= [filename1(1), mean(threshold_value), mean(avg_isi), mean(freq_in_hz), mean(total_count_Aps), mean(numbursts_list), mean(count_singletAPs), mean(avg_burst_duration), mean(freq_burst_in_Hz)];
...
singletAnalysisRow_final = mean(singletAnalysisRow_list);
